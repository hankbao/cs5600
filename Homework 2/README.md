...
```zsh
$ python scheduler.py -p SJF -j 3 -l 100,200,300 -c
ARG policy SJF
...
```zsh
$ python scheduler.py -p RR -j 3 -l 100,200,300 -q 1 -c
ARG policy RR
ARG jlist 100,200,300
...
**A**: Jobs with the same workloads.

---

**Q5**: For what types of workloads and quantum lengths does SJF deliver the same response times as RR?

**A**: Jobs with the same workloads will be the same with whatever quantum lengths for RR and SJF.

---

**Q6**: What happens to response time with SJF as job lengths increase?  Can you use the simulator to demonstrate the trend?

**A**: The response time with SJF will also increase.

```zsh
$ python scheduler.py -p SJF -j 3 -l 300,200,100 -c
ARG policy SJF
...
```zsh
$ python scheduler.py -p SJF -j 3 -l 400,300,200 -c
ARG policy SJF
ARG jlist 400,300,200
...
Execution trace:
  ...
  [ time 500 ] Run job 0 for 400.00 secs ( DONE at 900.00 )

Final statistics:
  Job   2 -- Response: 0.00  Turnaround 200.00  Wait 0.00
  Job   1 -- Response: 200.00  Turnaround 500.00  Wait 200.00
  Job   0 -- Response: 500.00  Turnaround 900.00  Wait 500.00

  Average -- Response: 233.33  Turnaround 533.33  Wait 233.33
```

```zsh
$ python scheduler.py -p SJF -j 3 -l 500,400,300 -c
ARG policy SJF
ARG jlist 500,400,300
...
Execution trace:
  [ time   0 ] Run job 2 for 300.00 secs ( DONE at 300.00 )
  [ time 300 ] Run job 1 for 400.00 secs ( DONE at 700.00 )
  [ time 700 ] Run job 0 for 500.00 secs ( DONE at 1200.00 )

Final statistics:
  Job   2 -- Response: 0.00  Turnaround 300.00  Wait 0.00
  Job   1 -- Response: 300.00  Turnaround 700.00  Wait 300.00
  Job   0 -- Response: 700.00  Turnaround 1200.00  Wait 700.00

  Average -- Response: 333.33  Turnaround 733.33  Wait 333.33
```
...
**A**: With increasing of the quantum lengths, the response time with RR becomes more like the response time with FIFO.

Suppose every job arrives at roughly the same time, and there are `N` jobs in total. If we assume each job gets a time slice once per round and the time slice (quantum) is `q`, then a job might have to wait for `(N-1)` time slices to get its first turn.

So the worst-case response time, `W`, could be:
